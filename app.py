...
# ==========================================
SHOE_STYLES = {
    "OXFORDS": "评审重点：作为最经典的鞋款，需平衡正式感与现代感。关注鞋楦线条的流畅度、布洛克雕花的视觉排布，以及在不同场所下的‘体面感’表达。",
    "SNEAKERS": "评审重点：强调轻盈、科技与都市动感。关注大底材质的视觉表达、鞋面网格的透气感展示，以及整体画面的活力氛围。",
    "DRESS SHOES": "评审重点：纯正商务属性。关注皮面反光的处理策划、缝线的严谨度，以及画面是否传达出绝对的专业与稳重。",
    "LOAFERS": "评审重点：松弛感与便利性。关注穿脱开口的舒适暗示、材质的柔韧度表达，以及半休闲状态下的优雅调性。",
    "BOOTS": "评审重点：轮廓美学与材质层次。关注鞋帮高度带来的保护感暗示、粗犷与精致的视觉冲突，以及硬朗的线条表达。",
    "SLIP-ONS": "评审重点：极致便利与日常舒适。关注弹力带/开口的细节、内里柔软度的视觉呈现，以及居家或通勤的亲和力。"
}

OCCASIONS = {
    "Daily Essentials": "视觉基调：亲切、高频、实穿。背景应具有生活气息，光影应自然均匀，文案应强调耐穿与百搭。",
    "Business-Ready": "视觉基调：权威、精干、专业。背景应简洁有力（如现代办公建筑），冷色调为主，文案应强调职场身份与品质感。",
    "Casual & Leisure": "视觉基调：松弛、自由、柔和。背景应具户外或咖啡馆气息，光线温润，文案应强调心情的释放与脚感的舒适。",
    "Special Occasions": "视觉基调：奢华、尊贵、戏剧性。背景应具高级感（如宴会大厅或暗调影棚），光影对比度高，强调独特性与社交溢价。"
}
...
is_leather = st.sidebar.toggle("3. 是否为真皮款式", value=False)
    ...
    if st.button("🚀 开始专家级深度评审", type="primary"):
        ...
        with status_container:
            ...
            try:
                ...
                model = genai.GenerativeModel('gemini-1.5-flash') # 使用稳定且极速的版本
                ...
                with col1:
                    st.download_button(label="📥 下载 TXT 报告", data=response.text, file_name=f"BM_{shoe_val}_Report.txt")
                with col2:
                    st.download_button(label="🖼️ 保存为 JPG 长图海报", data=jpg_data, file_name=f"BM_{shoe_val}_Poster.jpg", mime="image/jpeg")
